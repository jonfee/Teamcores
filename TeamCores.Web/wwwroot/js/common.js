...
function reload(params) {
    if (this instanceof Vue) {
        params = params || this.queries || null;
    }
 ...
 * @param {json} errorData 错误消息对象集合
 */
function apiError(code, errorData) {
    code = code || "ERROR";

    if (code === ApiResult.LOGIN_TIMEROUT.toString("name")) {
        location = "/home/login";
    } else if (code === ApiResult.NO_ACCESS.toString("name")) {
        ...
        if (code === ApiResult.BUSINISS_ERROR.toString("name")) {
            errorData = errorData || { error: "业务数据验证失败" };
            var keys = Object.keys(errorData);
            errMsg=errorData[keys[0]];
        }
